...
export default function TeamScreen() {
  ...
  useEffect(() => {
    ...
    function onStepChange({ step }) {
      if (step >= 2) setPhase(p => p === 'results' ? p : 'calculating')
    }
    ...
    api.get(`/api/sessions/${code}/info`)
      .then(res => {
        if (res.data.resultsRevealed) setPhase('results')
      })
      .catch(() => {})
  ...
  return <ResultsPhase group={group} teamResults={teamResults} sessionResults={sessionResults} />
}
